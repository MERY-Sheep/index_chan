...
use reporter::{print_report, generate_json_report};
use cleaner::Cleaner;
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Scan { directory, output, llm } => {
        ...
        Commands::Clean {
            ...
            println!("\n📊 結果:");
            println!("  削除: {}個 ({}行)", result.deleted_count, result.deleted_lines);
                ...
                for code in &dead_code {
                    ...
                    match llm_analyzer.analyze(&code.node, &context) {
                        Ok(analysis) => {
                            ...
                            analyses.insert(key, annotator::LLMAnalysisData {
        ...
        Commands::TestLlm { prompt, list_files, tokenizer_only } => {
            println!("🤖 LLM推論テスト開始\n");
            ...
            if tokenizer_only {
                ...
                if !tokenizer_path.exists() {
                    eprintln!("❌ tokenizer.jsonが見つかりません: {}", tokenizer_path.display());
                    return Ok(());
                }
                ...
                println!("🔤 エンコードテスト:");
                let encoding = tokenizer.encode(test_prompt.as_str(), true)
