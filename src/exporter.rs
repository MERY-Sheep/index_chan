...
impl GraphExporter {
    ...
    pub fn export_graphml(graph: &CodeGraph, output: &Path) -> Result<()> {
        ...
        writeln!(file, "         xsi:schemaLocation=\"http://graphml.graphdrawing.org/xmlns")?;
        writeln!(file, "         http://graphml.graphdrawing.org/xmlns/1.0/graphml.xsd\">")?;
        
        // Define attributes
        writeln!(file, "  <key id=\"name\" for=\"node\" attr.name=\"name\" attr.type=\"string\"/>")?;
        writeln!(file, "  <key id=\"type\" for=\"node\" attr.name=\"type\" attr.type=\"string\"/>")?;
        writeln!(file, "  <key id=\"file\" for=\"node\" attr.name=\"file_path\" attr.type=\"string\"/>")?;
        writeln!(file, "  <key id=\"exported\" for=\"node\" attr.name=\"is_exported\" attr.type=\"boolean\"/>")?;
        writeln!(file, "  <key id=\"used\" for=\"node\" attr.name=\"is_used\" attr.type=\"boolean\"/>")?;
        writeln!(file, "  <key id=\"edge_type\" for=\"edge\" attr.name=\"edge_type\" attr.type=\"string\"/>")?;
        ...
        for (id, node) in &graph.nodes {
            ...
            writeln!(file, "      <data key=\"name\">{}</data>", escape_xml(&node.name))?;
            writeln!(file, "      <data key=\"type\">{:?}</data>", node.node_type)?;
            writeln!(file, "      <data key=\"file\">{}</data>", escape_xml(&node.file_path.display().to_string()))?;
            writeln!(file, "      <data key=\"exported\">{}</data>", node.is_exported)?;
        ...
        for edge in &graph.edges {
            ...
            writeln!(file, "    <edge id=\"e{}\" source=\"n{}\" target=\"n{}\">", edge_count, edge.from, edge.to)?;
            writeln!(file, "      <data key=\"edge_type\">{:?}</data>", edge.edge_type)?;
            writeln!(file, "    </edge>")?;
            edge_count += 1;
        }
