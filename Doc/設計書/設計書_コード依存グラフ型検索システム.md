# コード依存グラフ型検索システム 設計書

## 1. システムアーキテクチャ

### 1.1 全体構成

```
┌─────────────────────────────────────────────────┐
│              LLM インターフェース層               │
│  (自然言語クエリ受付 / コンテキスト提供)          │
└─────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────┐
│              検索オーケストレーション層            │
│  (ベクトル検索 + グラフ探索の統合制御)            │
└─────────────────────────────────────────────────┘
         ↓                              ↓
┌──────────────────┐          ┌──────────────────┐
│ ベクトル検索エンジン │          │ グラフ探索エンジン │
│  (起点特定)        │          │  (依存関係追跡)   │
└──────────────────┘          └──────────────────┘
         ↓                              ↓
┌─────────────────────────────────────────────────┐
│                インデックス層                    │
│  (ベクトルDB / 依存グラフDB)                      │
└─────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────┐
│                解析・構築層                      │
│  (tree-sitter / AST解析 / グラフ構築)            │
└─────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────┐
│                コードベース                      │
└─────────────────────────────────────────────────┘
```

### 1.2 技術スタック

**コア実装**
- 言語: Rust
- 理由: 高速性、メモリ安全性、並列処理の容易さ

**解析基盤**
- tree-sitter: 多言語パーサー
- 各言語の tree-sitter grammar

**ベクトル検索**
- Qdrant または Milvus: ベクトルDB
- sentence-transformers: 埋め込みモデル（CodeBERT, GraphCodeBERT など）

**グラフ処理**
- petgraph: Rust のグラフライブラリ
- 永続化: RocksDB または SQLite

**API/CLI**
- axum: Web フレームワーク（REST API）
- clap: CLI フレームワーク

## 2. データモデル

### 2.1 コードエンティティ

```rust
// ノードの種類
enum NodeType {
    Function,
    Class,
    Method,
    Interface,
    Type,
    Module,
    File,
}

// コードノード
struct CodeNode {
    id: String,              // 一意識別子
    node_type: NodeType,     // ノード種別
    name: String,            // 関数名/クラス名など
    file_path: String,       // ファイルパス
    start_line: u32,         // 開始行
    end_line: u32,           // 終了行
    signature: String,       // 関数シグネチャなど
    docstring: Option<String>, // ドキュメント
    code_snippet: String,    // コード本体
    embedding: Vec<f32>,     // ベクトル埋め込み
}
```

### 2.2 依存関係エッジ

```rust
// エッジの種類
enum EdgeType {
    Calls,          // 関数呼び出し
    CalledBy,       // 呼び出される
    References,     // 参照
    Implements,     // 実装
    Extends,        // 継承
    Imports,        // インポート
    TypeOf,         // 型関係
}

// 依存関係エッジ
struct DependencyEdge {
    from_node: String,       // 起点ノードID
    to_node: String,         // 終点ノードID
    edge_type: EdgeType,     // エッジ種別
    weight: f32,             // 重み（優先度計算用）
}
```

### 2.3 依存グラフ

```rust
struct DependencyGraph {
    nodes: HashMap<String, CodeNode>,
    edges: Vec<DependencyEdge>,
    adjacency_list: HashMap<String, Vec<String>>, // 高速探索用
}
```

## 3. コア機能の詳細設計

### 3.1 解析・インデックス構築

#### 3.1.1 コード解析フロー

```
1. ファイル読み込み
   ↓
2. tree-sitter でパース → AST 生成
   ↓
3. AST トラバース
   - 関数/クラス定義の抽出
   - 呼び出し関係の抽出
   - import/export の抽出
   ↓
4. CodeNode 生成
   ↓
5. 埋め込みベクトル生成
   - 関数名 + シグネチャ + docstring を結合
   - CodeBERT でエンコード
   ↓
6. ベクトルDB に保存
```

#### 3.1.2 依存グラフ構築

```rust
fn build_dependency_graph(files: Vec<FilePath>) -> DependencyGraph {
    let mut graph = DependencyGraph::new();
    
    // Phase 1: ノード抽出
    for file in files {
        let ast = parse_file(file);
        let nodes = extract_nodes(ast);
        graph.add_nodes(nodes);
    }
    
    // Phase 2: エッジ構築
    for file in files {
        let ast = parse_file(file);
        let edges = extract_dependencies(ast, &graph.nodes);
        graph.add_edges(edges);
    }
    
    // Phase 3: インデックス構築
    graph.build_adjacency_list();
    
    graph
}
```

#### 3.1.3 インクリメンタル更新

```rust
fn incremental_update(graph: &mut DependencyGraph, changed_files: Vec<FilePath>) {
    // 変更ファイルに関連するノード・エッジを削除
    for file in &changed_files {
        graph.remove_nodes_by_file(file);
        graph.remove_edges_by_file(file);
    }
    
    // 再解析して追加
    for file in changed_files {
        let ast = parse_file(file);
        let nodes = extract_nodes(ast);
        let edges = extract_dependencies(ast, &graph.nodes);
        graph.add_nodes(nodes);
        graph.add_edges(edges);
    }
    
    // インデックス再構築
    graph.rebuild_adjacency_list();
}
```

### 3.2 ハイブリッド検索

#### 3.2.1 検索フロー全体

```rust
struct SearchRequest {
    query: String,           // 自然言語クエリ
    top_k: usize,           // 起点候補数（デフォルト: 3）
    max_hops_forward: usize, // 呼び出し先の最大ホップ数（デフォルト: 2）
    max_hops_backward: usize,// 呼び出し元の最大ホップ数（デフォルト: 1）
    max_tokens: usize,      // 最大トークン数（デフォルト: 8000）
}

struct SearchResult {
    contexts: Vec<ContextGroup>, // 起点ごとのコンテキスト
    total_tokens: usize,
}

struct ContextGroup {
    anchor_node: CodeNode,       // 起点ノード
    related_nodes: Vec<CodeNode>, // 関連ノード（優先度順）
    relevance_score: f32,        // 起点の関連度スコア
}
```

#### 3.2.2 ベクトル検索による起点特定

```rust
fn find_anchor_nodes(query: &str, top_k: usize) -> Vec<(CodeNode, f32)> {
    // 1. クエリを埋め込みベクトルに変換
    let query_embedding = encode_query(query);
    
    // 2. ベクトルDB で類似検索
    let results = vector_db.search(query_embedding, top_k);
    
    // 3. スコア付きで返す
    results.into_iter()
        .map(|(node_id, score)| {
            let node = graph.get_node(node_id);
            (node, score)
        })
        .collect()
}
```

#### 3.2.3 依存グラフ探索

```rust
fn explore_dependencies(
    graph: &DependencyGraph,
    anchor: &CodeNode,
    max_hops_forward: usize,
    max_hops_backward: usize,
) -> Vec<CodeNode> {
    let mut visited = HashSet::new();
    let mut result = Vec::new();
    
    // 起点自身を追加
    result.push(anchor.clone());
    visited.insert(anchor.id.clone());
    
    // 呼び出し先方向の探索（BFS）
    let forward_nodes = bfs_forward(
        graph,
        &anchor.id,
        max_hops_forward,
        &mut visited
    );
    result.extend(forward_nodes);
    
    // 呼び出し元方向の探索（BFS）
    let backward_nodes = bfs_backward(
        graph,
        &anchor.id,
        max_hops_backward,
        &mut visited
    );
    result.extend(backward_nodes);
    
    // 型定義の追加
    let type_nodes = find_related_types(graph, &result);
    result.extend(type_nodes);
    
    result
}

fn bfs_forward(
    graph: &DependencyGraph,
    start: &str,
    max_hops: usize,
    visited: &mut HashSet<String>,
) -> Vec<CodeNode> {
    let mut queue = VecDeque::new();
    let mut result = Vec::new();
    
    queue.push_back((start.to_string(), 0));
    
    while let Some((node_id, depth)) = queue.pop_front() {
        if depth >= max_hops {
            continue;
        }
        
        // 呼び出し先を取得
        let callees = graph.get_callees(&node_id);
        
        for callee_id in callees {
            if visited.insert(callee_id.clone()) {
                let node = graph.get_node(&callee_id);
                result.push(node);
                queue.push_back((callee_id, depth + 1));
            }
        }
    }
    
    result
}
```

#### 3.2.4 優先度付けとフィルタリング

```rust
fn prioritize_nodes(nodes: Vec<CodeNode>, anchor: &CodeNode) -> Vec<CodeNode> {
    let mut scored_nodes: Vec<(CodeNode, f32)> = nodes
        .into_iter()
        .map(|node| {
            let score = calculate_priority(&node, anchor);
            (node, score)
        })
        .collect();
    
    // スコア降順でソート
    scored_nodes.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    
    scored_nodes.into_iter().map(|(node, _)| node).collect()
}

fn calculate_priority(node: &CodeNode, anchor: &CodeNode) -> f32 {
    let mut score = 0.0;
    
    // 同一ファイル: +10
    if node.file_path == anchor.file_path {
        score += 10.0;
    }
    
    // 直接呼び出し: +5
    if is_direct_dependency(node, anchor) {
        score += 5.0;
    }
    
    // 型定義: +3
    if node.node_type == NodeType::Type || node.node_type == NodeType::Interface {
        score += 3.0;
    }
    
    // コードサイズが小さい: +1
    let lines = node.end_line - node.start_line;
    if lines < 20 {
        score += 1.0;
    }
    
    score
}

fn filter_by_token_limit(nodes: Vec<CodeNode>, max_tokens: usize) -> Vec<CodeNode> {
    let mut result = Vec::new();
    let mut total_tokens = 0;
    
    for node in nodes {
        let tokens = estimate_tokens(&node.code_snippet);
        if total_tokens + tokens <= max_tokens {
            result.push(node);
            total_tokens += tokens;
        } else {
            break;
        }
    }
    
    result
}
```

#### 3.2.5 複数起点の統合

```rust
fn merge_contexts(context_groups: Vec<ContextGroup>) -> SearchResult {
    // 重複ノードの排除（起点情報は保持）
    let mut node_map: HashMap<String, (CodeNode, Vec<String>)> = HashMap::new();
    
    for group in &context_groups {
        for node in &group.related_nodes {
            node_map
                .entry(node.id.clone())
                .or_insert_with(|| (node.clone(), Vec::new()))
                .1
                .push(group.anchor_node.id.clone());
        }
    }
    
    // 起点ごとにグループ化して返す
    SearchResult {
        contexts: context_groups,
        total_tokens: calculate_total_tokens(&context_groups),
    }
}
```

### 3.3 LLM インターフェース

#### 3.3.1 コンテキスト生成

```rust
fn format_context_for_llm(result: SearchResult) -> String {
    let mut output = String::new();
    
    output.push_str("# 検索結果\n\n");
    
    for (i, group) in result.contexts.iter().enumerate() {
        output.push_str(&format!("## 起点 {}: {}\n", i + 1, group.anchor_node.name));
        output.push_str(&format!("関連度スコア: {:.2}\n\n", group.relevance_score));
        
        // 起点ノード
        output.push_str("### 起点コード\n");
        output.push_str(&format_code_block(&group.anchor_node));
        output.push_str("\n");
        
        // 関連ノード
        output.push_str("### 関連コード\n");
        for node in &group.related_nodes {
            output.push_str(&format_code_block(node));
            output.push_str("\n");
        }
    }
    
    output
}

fn format_code_block(node: &CodeNode) -> String {
    format!(
        "```\n// ファイル: {}\n// 行: {}-{}\n{}\n```\n",
        node.file_path,
        node.start_line,
        node.end_line,
        node.code_snippet
    )
}
```

## 4. 統合コンテキスト編集システム

### 4.1 アーキテクチャ

```
自然言語クエリ（例: "関数名を変更"）
  ↓
ハイブリッド検索で関連ファイル特定
  ↓
統合コンテキスト生成
  ↓
LLM に渡して変更計画を生成
  ↓
変更計画をパース
  ↓
一括マージ・適用
```

### 4.2 データ構造

```rust
// 統合コンテキスト
struct UnifiedContext {
    files: Vec<FileContext>,
    total_tokens: usize,
}

struct FileContext {
    path: String,
    content: String,
    nodes: Vec<CodeNode>,  // このファイル内のノード
}

// 変更計画
struct ChangePlan {
    changes: Vec<FileChange>,
}

struct FileChange {
    file_path: String,
    edits: Vec<Edit>,
}

struct Edit {
    start_line: u32,
    end_line: u32,
    old_content: String,
    new_content: String,
    reason: String,  // 変更理由
}
```

### 4.3 統合コンテキスト生成

```rust
fn create_unified_context(
    graph: &DependencyGraph,
    anchor_nodes: Vec<CodeNode>,
) -> UnifiedContext {
    let mut file_map: HashMap<String, Vec<CodeNode>> = HashMap::new();
    
    // 起点ノードとその依存先を収集
    for anchor in anchor_nodes {
        let related = explore_dependencies(graph, &anchor, 2, 1);
        
        for node in related {
            file_map
                .entry(node.file_path.clone())
                .or_insert_with(Vec::new)
                .push(node);
        }
    }
    
    // ファイルごとにコンテキストを構築
    let files: Vec<FileContext> = file_map
        .into_iter()
        .map(|(path, nodes)| {
            let content = read_file(&path);
            FileContext { path, content, nodes }
        })
        .collect();
    
    let total_tokens = files.iter()
        .map(|f| estimate_tokens(&f.content))
        .sum();
    
    UnifiedContext { files, total_tokens }
}

// LLM 用にフォーマット
fn format_unified_context(context: &UnifiedContext) -> String {
    let mut output = String::new();
    
    output.push_str("# 統合コンテキスト\n\n");
    output.push_str(&format!("総ファイル数: {}\n", context.files.len()));
    output.push_str(&format!("総トークン数: {}\n\n", context.total_tokens));
    
    for file in &context.files {
        output.push_str(&format!("## ファイル: {}\n\n", file.path));
        output.push_str("```\n");
        output.push_str(&file.content);
        output.push_str("\n```\n\n");
        
        // 重要なノードを強調
        if !file.nodes.is_empty() {
            output.push_str("### 重要な要素\n");
            for node in &file.nodes {
                output.push_str(&format!("- {} (行 {}-{})\n", 
                    node.name, node.start_line, node.end_line));
            }
            output.push_str("\n");
        }
    }
    
    output
}
```

### 4.4 変更計画の生成と適用

```rust
// LLM に変更を依頼
async fn generate_change_plan(
    context: &UnifiedContext,
    instruction: &str,
) -> ChangePlan {
    let prompt = format!(
        "{}\n\n指示: {}\n\n以下の形式で変更計画を出力してください:\n\
        FILE: <ファイルパス>\n\
        EDIT: <開始行>-<終了行>\n\
        OLD:\n<古いコード>\n\
        NEW:\n<新しいコード>\n\
        REASON: <変更理由>\n",
        format_unified_context(context),
        instruction
    );
    
    let response = llm_client.complete(prompt).await;
    parse_change_plan(&response)
}

// 変更計画のパース
fn parse_change_plan(llm_output: &str) -> ChangePlan {
    let mut changes = Vec::new();
    let mut current_file = None;
    let mut current_edit = None;
    
    for line in llm_output.lines() {
        if line.starts_with("FILE:") {
            if let Some(file) = current_file.take() {
                changes.push(file);
            }
            current_file = Some(FileChange {
                file_path: line[5..].trim().to_string(),
                edits: Vec::new(),
            });
        } else if line.starts_with("EDIT:") {
            // パース処理...
        }
        // ... 他のパース処理
    }
    
    if let Some(file) = current_file {
        changes.push(file);
    }
    
    ChangePlan { changes }
}

// 変更の一括適用
fn apply_change_plan(plan: ChangePlan) -> Result<(), Error> {
    // トランザクション的に適用
    let mut backups = HashMap::new();
    
    // 1. バックアップ作成
    for change in &plan.changes {
        let content = read_file(&change.file_path)?;
        backups.insert(change.file_path.clone(), content);
    }
    
    // 2. 変更適用
    for change in &plan.changes {
        match apply_file_changes(&change.file_path, &change.edits) {
            Ok(_) => {},
            Err(e) => {
                // ロールバック
                for (path, content) in backups {
                    write_file(&path, &content)?;
                }
                return Err(e);
            }
        }
    }
    
    Ok(())
}

fn apply_file_changes(file_path: &str, edits: &[Edit]) -> Result<(), Error> {
    let content = read_file(file_path)?;
    let lines: Vec<&str> = content.lines().collect();
    let mut new_lines = Vec::new();
    
    let mut current_line = 0;
    
    for edit in edits {
        // 変更前の行をコピー
        new_lines.extend_from_slice(&lines[current_line..edit.start_line as usize]);
        
        // 新しいコードを挿入
        new_lines.extend(edit.new_content.lines());
        
        current_line = edit.end_line as usize;
    }
    
    // 残りの行をコピー
    new_lines.extend_from_slice(&lines[current_line..]);
    
    write_file(file_path, &new_lines.join("\n"))
}
```

### 4.5 変更のプレビューと確認

```rust
// 変更内容を diff 形式で表示
fn preview_changes(plan: &ChangePlan) -> String {
    let mut output = String::new();
    
    for change in &plan.changes {
        output.push_str(&format!("\n=== {} ===\n", change.file_path));
        
        for edit in &change.edits {
            output.push_str(&format!("\n@@ 行 {}-{} @@\n", 
                edit.start_line, edit.end_line));
            
            // 削除行
            for line in edit.old_content.lines() {
                output.push_str(&format!("- {}\n", line));
            }
            
            // 追加行
            for line in edit.new_content.lines() {
                output.push_str(&format!("+ {}\n", line));
            }
            
            output.push_str(&format!("\n理由: {}\n", edit.reason));
        }
    }
    
    output
}

// ユーザー確認
fn confirm_and_apply(plan: ChangePlan) -> Result<(), Error> {
    println!("{}", preview_changes(&plan));
    println!("\n変更を適用しますか? (y/n): ");
    
    let mut input = String::new();
    std::io::stdin().read_line(&mut input)?;
    
    if input.trim().to_lowercase() == "y" {
        apply_change_plan(plan)?;
        println!("変更を適用しました");
    } else {
        println!("変更をキャンセルしました");
    }
    
    Ok(())
}
```

## 5. API 設計

### 5.1 REST API

```rust
// 検索エンドポイント
POST /api/search
Request:
{
    "query": "ユーザー認証の処理",
    "top_k": 3,
    "max_hops_forward": 2,
    "max_hops_backward": 1,
    "max_tokens": 8000
}

Response:
{
    "contexts": [
        {
            "anchor_node": {
                "id": "func_123",
                "name": "authenticateUser",
                "file_path": "src/auth.ts",
                "code_snippet": "...",
                ...
            },
            "related_nodes": [...],
            "relevance_score": 0.92
        }
    ],
    "total_tokens": 6543
}

// 統合コンテキスト編集
POST /api/edit
Request:
{
    "query": "authenticateUser を verifyUser に変更",
    "top_k": 3,
    "preview_only": false  // true の場合は適用せずプレビューのみ
}

Response:
{
    "change_plan": {
        "changes": [
            {
                "file_path": "src/auth.ts",
                "edits": [
                    {
                        "start_line": 45,
                        "end_line": 45,
                        "old_content": "function authenticateUser()",
                        "new_content": "function verifyUser()",
                        "reason": "関数名の変更"
                    }
                ]
            }
        ]
    },
    "preview": "=== src/auth.ts ===\n- function authenticateUser()\n+ function verifyUser()\n...",
    "affected_files": 4
}

// インデックス構築
POST /api/index/build
Request:
{
    "project_path": "/path/to/project",
    "languages": ["typescript", "python"]
}

// インクリメンタル更新
POST /api/index/update
Request:
{
    "changed_files": ["src/auth.ts", "src/user.ts"]
}
```

### 5.2 CLI

```bash
# インデックス構築
code-graph index build --path ./my-project --languages typescript,python

# 検索
code-graph search "ユーザー認証の処理" --top-k 3 --max-tokens 8000

# 統合コンテキスト編集
code-graph edit "authenticateUser を verifyUser に変更" --preview
code-graph edit "authenticateUser を verifyUser に変更" --apply

# リファクタリング
code-graph refactor "この関数を3つに分割" --target src/auth.ts:45

# インクリメンタル更新
code-graph index update --files src/auth.ts src/user.ts

# グラフ可視化
code-graph visualize --node func_123 --depth 2
```

## 6. パフォーマンス最適化

### 6.1 並列処理

```rust
// ファイル解析の並列化
fn parse_files_parallel(files: Vec<FilePath>) -> Vec<CodeNode> {
    files
        .par_iter()  // rayon による並列イテレータ
        .flat_map(|file| {
            let ast = parse_file(file);
            extract_nodes(ast)
        })
        .collect()
}

// 複数起点の探索を並列化
fn explore_all_anchors_parallel(
    graph: &DependencyGraph,
    anchors: Vec<CodeNode>,
    config: SearchConfig,
) -> Vec<ContextGroup> {
    anchors
        .par_iter()
        .map(|anchor| {
            let nodes = explore_dependencies(graph, anchor, config);
            ContextGroup {
                anchor_node: anchor.clone(),
                related_nodes: nodes,
                relevance_score: anchor.relevance_score,
            }
        })
        .collect()
}
```

### 6.2 キャッシング

```rust
struct SearchCache {
    // クエリ → 起点ノードのキャッシュ
    anchor_cache: LruCache<String, Vec<(CodeNode, f32)>>,
    
    // ノードID → 探索結果のキャッシュ
    exploration_cache: LruCache<String, Vec<CodeNode>>,
}

impl SearchCache {
    fn get_or_compute_anchors<F>(
        &mut self,
        query: &str,
        compute: F,
    ) -> Vec<(CodeNode, f32)>
    where
        F: FnOnce() -> Vec<(CodeNode, f32)>,
    {
        if let Some(cached) = self.anchor_cache.get(query) {
            return cached.clone();
        }
        
        let result = compute();
        self.anchor_cache.put(query.to_string(), result.clone());
        result
    }
}
```

### 6.3 インデックス最適化

```rust
// 隣接リストの事前構築
struct OptimizedGraph {
    nodes: HashMap<String, CodeNode>,
    
    // 高速探索用の隣接リスト
    callees: HashMap<String, Vec<String>>,      // 呼び出し先
    callers: HashMap<String, Vec<String>>,      // 呼び出し元
    type_refs: HashMap<String, Vec<String>>,    // 型参照
}

// RocksDB による永続化
fn save_graph_to_disk(graph: &DependencyGraph, path: &str) {
    let db = rocksdb::DB::open_default(path).unwrap();
    
    // ノードをシリアライズして保存
    for (id, node) in &graph.nodes {
        let serialized = bincode::serialize(node).unwrap();
        db.put(format!("node:{}", id), serialized).unwrap();
    }
    
    // エッジも同様に保存
    for edge in &graph.edges {
        let serialized = bincode::serialize(edge).unwrap();
        db.put(format!("edge:{}", edge.from_node), serialized).unwrap();
    }
}
```

## 7. テスト戦略

### 7.1 単体テスト

```rust
#[cfg(test)]
mod tests {
    use super::*;
    
    #[test]
    fn test_bfs_forward() {
        let graph = create_test_graph();
        let mut visited = HashSet::new();
        
        let result = bfs_forward(&graph, "func_a", 2, &mut visited);
        
        assert_eq!(result.len(), 3);
        assert!(result.iter().any(|n| n.name == "func_b"));
    }
    
    #[test]
    fn test_priority_calculation() {
        let anchor = create_test_node("func_a", "file1.ts");
        let same_file = create_test_node("func_b", "file1.ts");
        let diff_file = create_test_node("func_c", "file2.ts");
        
        let score1 = calculate_priority(&same_file, &anchor);
        let score2 = calculate_priority(&diff_file, &anchor);
        
        assert!(score1 > score2);
    }
}
```

### 7.2 統合テスト

```rust
#[test]
fn test_end_to_end_search() {
    // 1. テストプロジェクトのインデックス構築
    let project_path = "tests/fixtures/sample_project";
    let graph = build_dependency_graph(project_path);
    
    // 2. 検索実行
    let request = SearchRequest {
        query: "user authentication".to_string(),
        top_k: 3,
        max_hops_forward: 2,
        max_hops_backward: 1,
        max_tokens: 8000,
    };
    
    let result = hybrid_search(&graph, request);
    
    // 3. 検証
    assert!(!result.contexts.is_empty());
    assert!(result.total_tokens <= 8000);
    assert!(result.contexts[0].anchor_node.name.contains("auth"));
}
```

### 7.3 統合コンテキスト編集のテスト

```rust
#[test]
fn test_unified_context_edit() {
    // 1. テストプロジェクトの準備
    let project = create_test_project();
    let graph = build_dependency_graph(&project);
    
    // 2. 統合コンテキスト生成
    let anchors = find_anchor_nodes("authenticateUser", 1);
    let context = create_unified_context(&graph, anchors);
    
    // 3. 変更計画生成（モック）
    let plan = ChangePlan {
        changes: vec![
            FileChange {
                file_path: "src/auth.ts".to_string(),
                edits: vec![
                    Edit {
                        start_line: 45,
                        end_line: 45,
                        old_content: "function authenticateUser()".to_string(),
                        new_content: "function verifyUser()".to_string(),
                        reason: "関数名変更".to_string(),
                    }
                ],
            }
        ],
    };
    
    // 4. 適用
    apply_change_plan(plan).unwrap();
    
    // 5. 検証
    let content = read_file("src/auth.ts");
    assert!(content.contains("function verifyUser()"));
    assert!(!content.contains("function authenticateUser()"));
}

#[test]
fn test_change_plan_rollback() {
    let project = create_test_project();
    
    // 意図的にエラーを起こす変更計画
    let plan = create_invalid_change_plan();
    
    let original_content = read_file("src/auth.ts");
    
    // 適用は失敗するはず
    let result = apply_change_plan(plan);
    assert!(result.is_err());
    
    // ロールバックされているか確認
    let current_content = read_file("src/auth.ts");
    assert_eq!(original_content, current_content);
}
```

### 7.4 精度評価

```rust
// 正解データセットを用意
struct GroundTruth {
    query: String,
    expected_functions: Vec<String>, // 期待される関数名
}

fn evaluate_precision_recall(
    graph: &DependencyGraph,
    test_cases: Vec<GroundTruth>,
) -> (f32, f32) {
    let mut total_precision = 0.0;
    let mut total_recall = 0.0;
    
    for case in test_cases {
        let result = hybrid_search(graph, case.query);
        let retrieved: HashSet<_> = result.contexts
            .iter()
            .flat_map(|g| g.related_nodes.iter().map(|n| &n.name))
            .collect();
        
        let expected: HashSet<_> = case.expected_functions.iter().collect();
        
        let tp = retrieved.intersection(&expected).count() as f32;
        let precision = tp / retrieved.len() as f32;
        let recall = tp / expected.len() as f32;
        
        total_precision += precision;
        total_recall += recall;
    }
    
    let n = test_cases.len() as f32;
    (total_precision / n, total_recall / n)
}
```

## 8. デプロイメント

### 8.1 構成

```
┌─────────────┐
│   CLI Tool  │  (ローカル実行)
└─────────────┘

または

┌─────────────┐      ┌──────────────┐
│  IDE Plugin │ ───> │  API Server  │
└─────────────┘      └──────────────┘
                            │
                     ┌──────┴──────┐
                     │             │
              ┌──────▼─────┐ ┌────▼─────┐
              │ Vector DB  │ │ Graph DB │
              └────────────┘ └──────────┘
```

### 8.2 設定ファイル

```toml
# config.toml

[project]
path = "./my-project"
languages = ["typescript", "python", "rust"]

[indexing]
incremental = true
watch_mode = false
exclude_patterns = ["node_modules", "dist", "build"]

[search]
default_top_k = 3
max_hops_forward = 2
max_hops_backward = 1
max_tokens = 8000

[vector_db]
type = "qdrant"
host = "localhost"
port = 6333

[embedding]
model = "microsoft/graphcodebert-base"
device = "cpu"  # or "cuda"

[performance]
num_threads = 8
cache_size_mb = 512
```

## 9. 監視・ログ

```rust
// メトリクス収集
struct Metrics {
    search_latency: Histogram,
    index_build_time: Histogram,
    cache_hit_rate: Counter,
    graph_size: Gauge,
}

// ログ出力
fn log_search(query: &str, result: &SearchResult, duration: Duration) {
    info!(
        query = query,
        num_contexts = result.contexts.len(),
        total_tokens = result.total_tokens,
        duration_ms = duration.as_millis(),
        "Search completed"
    );
}
```

## 10. Graph Keeper: コード健全性管理システム

### 10.1 アーキテクチャ

```
┌─────────────────────────────────────┐
│      大型 LLM (GPT-4 など)          │
│   (コード生成・編集)                 │
└─────────────────┬───────────────────┘
                  │
                  │ 相談・検証
                  ↓
┌─────────────────────────────────────┐
│   Graph Keeper (小型特化型 LLM)      │
│   - 整合性チェック                   │
│   - デッドコード検出                 │
│   - 最適化提案                       │
└─────────────────┬───────────────────┘
                  │
                  │ 監視・更新
                  ↓
┌─────────────────────────────────────┐
│        依存グラフ DB                 │
└─────────────────────────────────────┘
```

### 10.2 デッドコード検出

```rust
// デッドコード検出の種類
enum DeadCodeType {
    Orphaned,       // 完全に使われていない
    Zombie,         // 何もしていない
    UnusedImport,   // 未使用の import
    Unreachable,    // 到達不可能
    Deprecated,     // 古いバージョン
    Untested,       // テストがない
}

struct CodeHealthReport {
    dead_code: Vec<DeadCodeItem>,
    total_waste_lines: usize,
    cleanup_tasks: Vec<CleanupTask>,
    technical_debt_score: f32,
}

struct DeadCodeItem {
    node: CodeNode,
    dead_type: DeadCodeType,
    reason: String,
    last_modified: DateTime,
}

struct CleanupTask {
    item: DeadCodeItem,
    priority: Priority,      // High, Medium, Low
    risk: Risk,              // None, Low, Medium, High
    estimated_impact: String,
}
```

### 10.3 検出アルゴリズム

```rust
impl GraphKeeper {
    // 孤立ノード（デッドコード）の検出
    fn find_orphaned_nodes(&self, graph: &DependencyGraph) -> Vec<CodeNode> {
        graph.nodes
            .values()
            .filter(|node| {
                // 呼び出し元が0個 = 誰も使っていない
                graph.get_callers(&node.id).is_empty() &&
                // エントリーポイントでもない
                !self.is_entry_point(node) &&
                // エクスポートされていない
                !self.is_exported(node)
            })
            .cloned()
            .collect()
    }
    
    // ゾンビコード（何もしていない）の検出
    fn find_zombie_code(&self, graph: &DependencyGraph) -> Vec<CodeNode> {
        graph.nodes
            .values()
            .filter(|node| {
                // 呼び出し先が0個（何もしていない）
                let callees = graph.get_callees(&node.id);
                callees.is_empty() ||
                // または単純なラッパー
                (callees.len() == 1 && self.is_simple_wrapper(node))
            })
            .cloned()
            .collect()
    }
    
    // 未使用 import の検出
    fn find_unused_imports(&self, graph: &DependencyGraph) -> Vec<String> {
        let mut unused = Vec::new();
        
        for file in graph.get_all_files() {
            let imports = graph.get_imports(&file);
            let used_symbols = graph.get_used_symbols(&file);
            
            for import in imports {
                if !used_symbols.contains(&import) {
                    unused.push(format!("{}:{}", file, import));
                }
            }
        }
        
        unused
    }
    
    // 古いバージョンの残骸検出
    fn find_deprecated_code(&self, graph: &DependencyGraph) -> Vec<CodeNode> {
        let mut deprecated = Vec::new();
        
        // 名前パターンで検出（v1, v2, old, legacy など）
        for node in graph.nodes.values() {
            if self.has_version_suffix(&node.name) ||
               self.has_deprecated_prefix(&node.name) {
                // 新しいバージョンが存在するか確認
                if self.has_newer_version(graph, node) {
                    deprecated.push(node.clone());
                }
            }
        }
        
        deprecated
    }
    
    // テストされていないコード検出
    fn find_untested_code(&self, graph: &DependencyGraph) -> Vec<CodeNode> {
        let test_files = graph.get_test_files();
        let production_code = graph.get_production_code();
        
        production_code
            .into_iter()
            .filter(|node| {
                // テストファイルから呼ばれていない
                !self.has_test_coverage(graph, node, &test_files)
            })
            .collect()
    }
    
    // 健全性レポート生成
    fn generate_health_report(&self, graph: &DependencyGraph) -> CodeHealthReport {
        let orphaned = self.find_orphaned_nodes(graph);
        let zombie = self.find_zombie_code(graph);
        let unused_imports = self.find_unused_imports(graph);
        let deprecated = self.find_deprecated_code(graph);
        let untested = self.find_untested_code(graph);
        
        let mut dead_code = Vec::new();
        
        // 各種デッドコードを統合
        for node in orphaned {
            dead_code.push(DeadCodeItem {
                node,
                dead_type: DeadCodeType::Orphaned,
                reason: "どこからも呼ばれていません".to_string(),
                last_modified: self.get_last_modified(&node),
            });
        }
        
        for node in zombie {
            dead_code.push(DeadCodeItem {
                node,
                dead_type: DeadCodeType::Zombie,
                reason: "何もしていない関数です".to_string(),
                last_modified: self.get_last_modified(&node),
            });
        }
        
        // 総行数計算
        let total_waste_lines = dead_code.iter()
            .map(|item| item.node.end_line - item.node.start_line)
            .sum();
        
        // クリーンアップタスク生成
        let cleanup_tasks = self.prioritize_cleanup(&dead_code);
        
        // 技術的負債スコア計算
        let technical_debt_score = self.calculate_debt_score(
            &dead_code,
            graph.total_lines(),
        );
        
        CodeHealthReport {
            dead_code,
            total_waste_lines,
            cleanup_tasks,
            technical_debt_score,
        }
    }
    
    // クリーンアップの優先度付け
    fn prioritize_cleanup(&self, dead_code: &[DeadCodeItem]) -> Vec<CleanupTask> {
        let mut tasks = Vec::new();
        
        for item in dead_code {
            let (priority, risk) = match item.dead_type {
                DeadCodeType::Orphaned => (Priority::High, Risk::None),
                DeadCodeType::UnusedImport => (Priority::High, Risk::None),
                DeadCodeType::Zombie => (Priority::Medium, Risk::Low),
                DeadCodeType::Deprecated => (Priority::Medium, Risk::Low),
                DeadCodeType::Unreachable => (Priority::Low, Risk::None),
                DeadCodeType::Untested => (Priority::Low, Risk::Medium),
            };
            
            tasks.push(CleanupTask {
                item: item.clone(),
                priority,
                risk,
                estimated_impact: self.estimate_impact(item),
            });
        }
        
        // 優先度順にソート
        tasks.sort_by_key(|t| (t.priority, t.risk));
        tasks
    }
}
```

### 10.4 自動クリーンアップ

```rust
fn auto_cleanup(
    graph: &mut DependencyGraph,
    report: &CodeHealthReport,
    dry_run: bool,
) -> CleanupResult {
    let mut removed_lines = 0;
    let mut removed_files = 0;
    let mut actions = Vec::new();
    
    for task in &report.cleanup_tasks {
        // リスクが高いものはスキップ
        if task.risk == Risk::High {
            continue;
        }
        
        // ユーザー確認が必要なものはスキップ
        if task.risk == Risk::Medium && !dry_run {
            println!("⚠️  要確認: {} - {}", task.item.node.name, task.item.reason);
            continue;
        }
        
        match task.item.dead_type {
            DeadCodeType::UnusedImport => {
                if !dry_run {
                    remove_import(&task.item.node);
                }
                actions.push(format!("✓ import 削除: {}", task.item.node.name));
            }
            DeadCodeType::Orphaned => {
                let lines = task.item.node.end_line - task.item.node.start_line;
                if !dry_run {
                    remove_code(&task.item.node);
                }
                removed_lines += lines;
                actions.push(format!("✓ 関数削除: {} ({}行)", task.item.node.name, lines));
            }
            _ => {}
        }
    }
    
    CleanupResult {
        removed_lines,
        removed_files,
        actions,
    }
}
```

### 10.5 CLI インターフェース

```bash
# コード健全性チェック
$ code-graph health-check

📊 コード健全性レポート
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🗑️  デッドコード: 23個 (456行)
├─ src/old-auth.ts:45-78 (oldAuthMethod)
│  理由: どこからも呼ばれていません
│  最終更新: 2023-03-15
├─ src/utils.ts:123-145 (deprecatedHelper)
│  理由: 新しいバージョンが存在します
│  最終更新: 2023-01-20
└─ src/legacy.ts:1-234 (ファイル全体)
   理由: 完全に未使用
   最終更新: 2022-11-10

🧟 ゾンビコード: 8個 (92行)
├─ src/logger.ts:34-38 (emptyLogger)
│  理由: 何もしていない関数
└─ src/wrapper.ts:12-15 (simpleWrapper)
   理由: 単純なラッパー関数

📦 未使用 import: 15個
├─ src/main.ts: oldFunction, deprecatedUtil
└─ src/app.ts: unusedHelper

⚠️  テストなし: 34個の関数

💾 削減可能な行数: 548行 (全体の 12%)
📉 技術的負債スコア: 7.2/10 (要改善)

🎯 推奨アクション:
1. [安全] src/legacy.ts を削除 (234行削減)
2. [安全] 未使用 import を削除 (15箇所)
3. [要確認] ゾンビコードを確認 (8箇所)

自動クリーンアップを実行しますか? (y/n)

# ドライラン（実際には削除しない）
$ code-graph cleanup --dry-run

# 自動クリーンアップ実行
$ code-graph cleanup --auto

🧹 クリーンアップ実行中...

✓ import 削除: oldFunction (src/main.ts)
✓ import 削除: deprecatedUtil (src/main.ts)
✓ 関数削除: oldAuthMethod (45行)
✓ ファイル削除: src/legacy.ts (234行)

📊 クリーンアップ完了
削減した行数: 548行
削減したファイル: 3個
技術的負債改善: 7.2 → 5.1 (-29%)
```

### 10.6 IDE 統合

```typescript
// VSCode 拡張での表示例
function oldAuthMethod() {  // ⚠️ デッドコード: どこからも呼ばれていません
    // ...                  // 💡 クリックして削除
}

import { oldFunction } from './utils';  // ⚠️ 未使用の import
                                        // 💡 クリックして削除

function emptyLogger() {  // 💀 ゾンビコード: 何もしていません
    return;               // 💡 クリックして削除
}
```

## 11. まとめ

本設計書では、ベクトル検索とグラフ探索を組み合わせたハイブリッド検索システム、統合コンテキストによる一括編集機能、および Graph Keeper によるコード健全性管理の詳細を定義した。

**主要な設計判断**:
- Rust による高性能実装
- tree-sitter による多言語対応
- トップk起点 + BFS探索の組み合わせ
- 優先度ベースのフィルタリング
- 統合コンテキストによる整合性の高い一括編集
- Graph Keeper による継続的なコード品質維持
- デッドコード自動検出とクリーンアップ
- トランザクション的な変更適用とロールバック機構
- インクリメンタル更新による高速化

**統合コンテキスト編集の利点**:
- 複数ファイルにまたがる変更を一度に処理
- LLM が全体を見るため整合性が保たれる
- 変更漏れのリスクを大幅に削減
- プレビュー機能による安全な適用

**Graph Keeper の利点**:
- リアルタイムでコードの健全性を監視
- デッドコード、ゾンビコード、未使用 import を自動検出
- 技術的負債の定量評価と可視化
- 安全性を考慮した優先度付きクリーンアップ
- コードベースの肥大化を防止

**次のステップ**:
1. MVP 実装（単一言語、基本機能のみ）
2. デッドコード検出アルゴリズムの実装と検証
3. 小規模プロジェクトでの精度検証
4. 統合コンテキスト編集の実用性検証
5. Graph Keeper の学習データ生成と訓練
6. パフォーマンスベンチマーク
7. 段階的な機能拡張
